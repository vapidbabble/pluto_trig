...
using Plots, PlutoUI, Pkg,Symbolics,HypertextLiteral
...
md"""
Triangles and circles might not to appear to get along. Triangles are full of straight lines and sharp angles. Circles are bendy, and are smooth. But Trigonometry is the music that allows them to dance together, despite their apparent differences.
"""

# ╔═╡ 79abd914-1f26-4164-9d0d-4de35edddc37
TableOfContents()

# ╔═╡ 600ef612-f271-480d-997a-55589b7b82d0
plotly()

# ╔═╡ bae3a1c7-fb11-4c0c-ba1a-6a6e135ffa7c
md"""
Let's see what happpens to sine and cosine on the unit circle as we vary the angle in radians.
"""

# ╔═╡ 8ca722f9-2a1c-4cce-8aad-26379bdb77c4
md"## Radian adjustment"

# ╔═╡ 362242e3-f02f-40a4-9a61-dc047d176f17
radians = @bind n Slider(0.000:0.0001:2π, 3.142, false);

# ╔═╡ 99300580-6f4f-4de7-9231-2d2c8dff82ca
@htl("""<div style="position: fixed;
   top: 0;
   right: 0;
   background: #336600;
   padding: 6px;
   color: #fff;
   font-size: 1.4em;">Radians $(radians)</div>
""")

# ╔═╡ 823f5c32-3801-40c9-a0f8-de2f5bd73116
md"## Values"

# ╔═╡ 693545ae-fbee-43ae-a4eb-27102a3eebbf
md"Angle is $(n) radians."

# ╔═╡ 20dda63e-b343-42c4-809d-19ab620dde57
md"Cosine (side adjacent) of $(n) is $(cos(n))"

# ╔═╡ 44900ec3-72d8-4664-bf5a-00883cafff9c
md"and sine (side opposite) is $(sin(n))."

# ╔═╡ c868ec19-5172-4223-b876-2645e1eac057
cos(n),sin(n)

# ╔═╡ a1b5bc4d-998f-4961-95cf-02d5b6788a98
md"## The Unit Circle"

# ╔═╡ fb57b26f-5f5e-481c-8cab-627e7de2b468
begin
	plot(cos, sin, 0, n, aspect_ratio=1,xlims=(-1.50, 1.50), ylims=(-1.50,1.50),label = "arc")
	plot!([0,cos(n)],[0,0],label = "side adj")
	plot!([cos(n),cos(n)],[0,sin(n)],label = "side opp")
	plot!([0,cos(n)],[0,sin(n)],label = "hypo")
	#plot!([0,1/2],[0,√3/2],label="π/3")
	#plot!([0,√3/2],[0,1/2],label="π/6",color="black")
	#plot!([0,√3/2],[0,1/2],label="π/6")
	#plot!([0,√2/2],[0,√2/2],label="π/4")
end

# ╔═╡ 077143cb-134d-46cd-b5e6-21dbd4e10ce1
n

# ╔═╡ d4606f76-568f-4fa7-b2d9-094772df3e9a
md"## Sine and Cosine"

# ╔═╡ 1dab92ac-7c70-416b-a584-985a186f55b1
begin
	plot([cos, sin], 0, n, aspect_ratio=1,layout=2,xlims=(0, 2π ), ylims=(-1.50,1.50),label = ["cosine" "sine"], color=[1 2])
	
end

# ╔═╡ 834fd994-14f4-48ae-8543-c88afae6ba07
md"# Experimentation"

# ╔═╡ b5594710-dc66-41a8-91d7-9bca89f84b39
plot(sin )

# ╔═╡ c288d4f5-67f9-4910-b51d-9a5f70a97fa2
begin
	f(x)=sin(x - π/2)
	f1(x)=-cos(x)
	plot(sin, 0, 2π, label= "sin(x)")
	plot!(f, 0, 2π, label= "sin(x - π/2)")
	plot!(f1, 0, 2π, label= "-cos(x)")
end

# ╔═╡ 7bd15590-6193-4a11-8032-c5317b5d575e
begin
	g(x)=cos(x - π/2)
	plot(cos, 0, 2π, label= "cos(x)")
	plot!(g, 0, 2π, label= "cos(x - π/2)")
end

# ╔═╡ d129a1e6-1ddd-46ea-a6f6-b842ae90f651
md"# Derivatives and Rates of Change"

# ╔═╡ a24c296b-04c2-4c79-baef-7d60d1f8af1f
begin
	@variables t
	Dt = Differential(t)
end

# ╔═╡ 0a1eb80a-5fe0-45c7-99ef-4bf600da742e
md" Derivative of sin(t) is $(expand_derivatives(Dt(sin(t))))"

# ╔═╡ b23f8b70-3ae1-4877-af28-9f6b8138be31
md"Derivative of $cos(t)$ is $(expand_derivatives(Dt(cos(t)))))"

# ╔═╡ 31b0b864-caef-4e8d-ba26-fc535ca68d12
+(1, 2, 3, 4)

# ╔═╡ 46374d2c-a52b-4c4a-8af0-76e0dd3f9061
md"""
Ctrl-M to enter Markdown, F1 for help
"""
...
manifest_format = "2.0"
project_hash = "bc976cd3566f750f0c5293367804dd0fe12ce306"
